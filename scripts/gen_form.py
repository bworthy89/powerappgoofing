...
o.write("# Generated by scripts/gen_form.py - create / edit any TB_* record.\n"
        ...
        # Leaving the form is when everything should be current, however you left it -
        # saved, deleted, or cancelled after the "+ New" detour created a model.
        #
        # Patch keeps the local cache in step only for the row it wrote, so Refresh is what
        # brings in anything else. And varCustomer / varInstallation hold a COPY taken when
        # the row was tapped: without re-reading them, a machine edited from its own screen
        # returns to a screen still showing the old values. That was the restart.
        #
        # Guarded on a real row - App.OnStart seeds both from Defaults(), whose ID is blank,
        # and looking that up would empty the variable.
        "      OnHidden: |\n"
        "        =Refresh(TB_Customers);\n"
        "        Refresh(TB_Products);\n"
        "        Refresh(TB_Installations);\n"
        "        Refresh(TB_SolutionUnits);\n"
        "        Refresh(TB_SoftwareVersions);\n"
        "        Refresh(TB_References);\n"
        "        If(varCustomer.ID > 0,\n"
        "            Set(varCustomer, LookUp(TB_Customers, ID = varCustomer.ID)));\n"
